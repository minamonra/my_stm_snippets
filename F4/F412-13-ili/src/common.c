...
uint8_t decode_utf8(const char **str) {
    ...
    (*str)++; // Сдвигаем указатель строки вперед
    
    if (c == 0xD0 || c == 0xD1) {
        uint8_t next_c = (uint8_t)(**str);
        if (next_c == 0) return 0;
        (*str)++; // Съедаем второй байт UTF-8
        
        if (c == 0xD0) {
            if (next_c == 0x81) return 144; // 'Ё' -> 'А' (заглушка)
            if (next_c >= 0x90 && next_c <= 0xBF) {
                // Математически точный сдвиг под вашу таблицу символов (А = 144)
                return next_c - 0x90 + 144; 
            }
        } else if (c == 0xD1) {
            if (next_c == 0x91) return 181; // 'ё' -> 'е'
            if (next_c >= 0x80 && next_c <= 0x8F) {
                // Математически точный сдвиг под вашу таблицу символов (р = 128)
                return next_c - 0x80 + 128; 
            }
        }
    }
    return c; // Возвращаем стандартный ASCII (английские буквы, цифры, знаки)
}
