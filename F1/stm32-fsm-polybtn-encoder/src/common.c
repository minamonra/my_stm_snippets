...
void SysTick_Handler(void)
{
  ...
  // Твои оригинальные серийные программные таймеры задержек
  if (ddms)   ddms--;
  if (pc13ms) pc13ms--;
  if (pc14ms) pc14ms--;
}

// Глухая задержка для инициализации и стартовых процедур
void delay_ms(uint16_t ms)
{
  ddms = ms;
  while (ddms) {
    // Ждем, пока прерывание SysTick_Handler опустит ddms до 0
    __NOP(); // Вставляем пустую ассемблерную инструкцию для стабильности цикла
  }
}
